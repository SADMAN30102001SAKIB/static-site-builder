...
export default function WebsiteDetails({ params }) {
  ...
  const [activities, setActivities] = useState([
    ...
    {
      id: '3',
      action: 'New page created: Contact',
  ...
  useEffect(() => {
    async function fetchWebsiteData() {
      try {
        ...
        setEditedName(data.website.name);
        setEditedDescription(data.website.description || '');
        
        // Fetch pages
        const pagesResponse = await fetch(`/api/websites/${id}/pages`);
  ...
  const handlePublishToggle = async () => {
    ...
    try {
      ...
      if (!response.ok) {
        throw new Error(website.published ? 'Failed to unpublish website' : 'Failed to publish website');
      }
      ...
      const newActivity = {
        ...
        action: website.published ? 'Website unpublished' : 'Website published',
  ...
  const handleSaveChanges = async () => {
    ...
    try {
      ...
      const newActivity = {
        id: Date.now().toString(),
        action: 'Website information updated',
        timestamp: new Date().toISOString(),
        status: 'success'
      };
      setActivities(prev => [newActivity, ...prev]);
    ...
    return (
      <Container maxWidth="max-w-6xl">
        <div className="flex flex-col items-center justify-center py-10">
          ...
          <p className="mt-4 text-gray-600 dark:text-gray-400">Loading website data...</p>
        </div>
      </Container>
    );
  }
                ...
                <div className="border-t border-gray-200 dark:border-gray-700 pt-4">
                  <dl className="grid grid-cols-1 sm:grid-cols-2 gap-x-4 gap-y-3">
                    <div>
                      ...
                      <dd className="mt-1 text-sm text-gray-900 dark:text-white">{formattedDate(website.createdAt)}</dd>
                    </div>
                    <div>
                      <dt className="text-sm font-medium text-gray-500 dark:text-gray-400">Last updated</dt>
                      <dd className="mt-1 text-sm text-gray-900 dark:text-white">{formattedDate(website.updatedAt)}</dd>
                    </div>
                    <div>
                    ...
                    <div>
                      ...
                      <dd className="mt-1 text-sm text-gray-900 dark:text-white">{pages.length}</dd>
                    </div>
                  </dl>
                </div>
            ...
            <div className="space-y-4">
              <div className="p-4 bg-red-50 dark:bg-red-900/20 rounded-lg">
                ...
                <p className="mt-1 text-xs text-red-700 dark:text-red-500">
                  Once deleted, all website data and pages will be permanently removed.
                </p>
                <div className="mt-3">
                  <Button
      ...
      {showDeleteModal && (
        <div className="fixed inset-0 bg-black bg-opacity-60 flex items-center justify-center z-50 p-4">
          <div className="bg-white dark:bg-gray-800 rounded-lg max-w-md w-full p-6 shadow-xl">
            ...
            <p className="text-gray-600 dark:text-gray-400 mb-4">
              Are you sure you want to delete <span className="font-semibold text-gray-900 dark:text-white">{website.name}</span>? This action cannot be undone and all website data will be permanently deleted.
            </p>
