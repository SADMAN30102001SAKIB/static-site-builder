...
const componentRenderers = {
  ...
  select: ({ properties }) => {
    ...
    return (
      <div>
        ...
        <select className="w-full px-3 py-2 border border-gray-300 dark:border-gray-600 rounded-md shadow-sm focus:outline-none focus:ring-[rgb(var(--primary))] focus:border-[rgb(var(--primary))] dark:bg-gray-700">
  ...
  contactForm: ({ properties }) => {
    const { title, submitButtonText, showNameField, showSubjectField } = properties;
    
    return (
      <div className="p-6 border border-dashed border-gray-300 dark:border-gray-600 rounded-lg">
        {title && <h3 className="text-lg font-medium mb-4">{title || 'Contact Us'}</h3>}
  ...
  hero: ({ properties, children }) => {
    const { title, subtitle, backgroundImage, textColor, alignment, height } = properties;
    ...
    return (
      ...
      >
        ...
        <div className={`relative z-10 p-8 w-full ${alignment === 'left' ? 'text-left' : alignment === 'right' ? 'text-right' : 'text-center'}`}>
          <h1 className="text-3xl md:text-4xl lg:text-5xl font-bold mb-4">{title || 'Hero Title'}</h1>
  ...
  features: ({ properties }) => {
    ...
    return (
      <div className="py-12 border border-dashed border-gray-300 dark:border-gray-600 rounded-lg">
        {title && (
          <h2 className="text-2xl font-bold text-center mb-8">{title || 'Our Features'}</h2>
        )}
        <div className={`grid grid-cols-1 md:grid-cols-${Math.min(count, 3)} gap-8 px-4`}>
          {Array.from({ length: count }).map((_, i) => (
            <div key={i} className="text-center p-4">
              ...
              <p className="text-gray-600 dark:text-gray-400">Description of feature {i + 1}</p>
            </div>
          ))}
        </div>
      </div>
  ...
  pricing: ({ properties }) => {
    ...
    return (
      <div className="py-12 border border-dashed border-gray-300 dark:border-gray-600 rounded-lg">
        ...
        <div className={`grid grid-cols-1 md:grid-cols-${Math.min(count, 3)} gap-8 px-4`}>
          {Array.from({ length: count }).map((_, i) => (
            <div key={i} className={`bg-white dark:bg-gray-800 rounded-lg shadow-md overflow-hidden ${i === 1 ? 'border-2 border-[rgb(var(--primary))]' : 'border border-gray-200 dark:border-gray-700'}`}>
              <div className={`p-6 ${i === 1 ? 'bg-[rgb(var(--primary))] text-white' : 'bg-gray-50 dark:bg-gray-700'}`}>
                <h3 className="text-xl font-bold mb-1">{planNames[i] || `Plan ${i + 1}`}</h3>
                <p className={`${i === 1 ? 'text-white/80' : 'text-gray-600 dark:text-gray-400'}`}>For small businesses</p>
              </div>
              <div className="p-6">
                <div className="flex items-baseline mb-4">
                  <span className="text-3xl font-bold">{currencySymbol}{prices[i] || (i + 1) * 10}</span>
                  <span className="text-gray-600 dark:text-gray-400 ml-1">/{billingPeriod}</span>
                </div>
                ...
                <button className={`w-full py-2 rounded-md ${i === 1 ? 'bg-[rgb(var(--primary))] text-white' : 'bg-gray-200 dark:bg-gray-700 text-gray-800 dark:text-white'}`}>
                  Choose Plan
                </button>
              </div>
            </div>
  ...
  gallery: ({ properties }) => {
    ...
    return (
      <div className="py-8 border border-dashed border-gray-300 dark:border-gray-600 rounded-lg">
        <div className={`grid grid-cols-1 sm:grid-cols-2 md:grid-cols-${colCount} gap-4 p-4`}>
          {Array.from({ length: imageCount }).map((_, i) => (
            <div key={i} className="aspect-w-16 aspect-h-9 overflow-hidden rounded-lg">
              <img 
    ...
    return (
      ...
      >
        ...
        <div className="hidden md:flex space-x-6">
          <a href="#" className="hover:text-[rgb(var(--primary))]">Home</a>
          <a href="#" className="hover:text-[rgb(var(--primary))]">About</a>
          <a href="#" className="hover:text-[rgb(var(--primary))]">Services</a>
          <a href="#" className="hover:text-[rgb(var(--primary))]">Contact</a>
        </div>
        ...
        <div className="max-w-7xl mx-auto px-4 py-12">
          <div className={`grid grid-cols-1 md:grid-cols-${colCount} gap-8`}>
            <div>
              ...
              <p className="text-gray-600 dark:text-gray-400">Short description of your company or website.</p>
            </div>
            <div>
              <h3 className="text-lg font-bold mb-4">Quick Links</h3>
              <ul className="space-y-2">
                {['Home', 'About', 'Services', 'Blog', 'Contact'].slice(0, 5).map((item, i) => (
                  <li key={i}><a href="#" className="text-gray-600 dark:text-gray-400 hover:text-[rgb(var(--primary))]">{item}</a></li>
                ))}
              </ul>
            </div>
            <div>
            ...
            {colCount >= 4 && (
              <div>
                ...
                <div className="flex">
                  <input type="email" placeholder="Your email" className="px-3 py-2 rounded-l-md w-full" />
                  <button className="bg-[rgb(var(--primary))] text-white px-4 py-2 rounded-r-md">Subscribe</button>
                </div>
              </div>
            )}
          </div>
          ...
          {showSocial !== false && (
            <div className="flex space-x-4 justify-center mt-8">
              {['Facebook', 'Twitter', 'Instagram', 'LinkedIn'].map((platform, i) => (
                <a key={i} href="#" className="text-gray-600 dark:text-gray-400 hover:text-[rgb(var(--primary))]">
  ...
  socialLinks: ({ properties }) => {
    ...
    const social = platforms || ['facebook', 'twitter', 'instagram', 'linkedin'];
    
    return (
      <div className={`flex ${layout === 'vertical' ? 'flex-col space-y-3' : 'space-x-3'}`}>
  ...
  callToAction: ({ properties }) => {
    ...
    return (
      ...
      >
        <h2 className="text-2xl font-bold mb-2">{title || 'Ready to get started?'}</h2>
        <p className="mb-6">{subtitle || 'Join thousands of satisfied customers today!'}</p>
...
export default function CanvasComponent({
  ...
  const [{ isOver, canDrop }, drop] = useDrop(() => ({
    ...
    drop: (item, monitor) => {
      ...
      // If we're dropping a new component from the sidebar
      if (item.type && !item.id) {
        // Get children count for positioning
  ...
  const renderComponent = componentRenderers[component.type] || defaultRenderer;
